...
contract Voter is Ownable2StepUpgradeable, IVoter {
    ...
    function onRegister() external override {
        ...
        for (uint256 i = 0; i < periods.length; ++i) {
            ...
            require(_bribesPerPriod[periods[i]][pool].length + 1 <= Constants.MAX_BRIBES_PER_POOL || msg.sender == owner(), "too much bribes");
    ...
    function _checkLockDurations(
        ...
        if (_remainingLockTime(position) < _periodDuration) {
            revert IVoter__InsufficientLockTime();
        }
    }
