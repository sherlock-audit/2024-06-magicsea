...
contract VoterTest is Test {
    ...
    function setUp() public {
        ...
        address poolImpl = address(
            new MlumStaking(_stakingToken, _rewardToken)
        );
            ...
            address(
                new TransparentUpgradeableProxy2Step(
                    ...
                    abi.encodeWithSelector(
                        MlumStaking.initialize.selector, DEV
                    )
                )
            )
        );
        ...
        address voterImpl = address(
            new Voter(mock, _pool, IRewarderFactory(address(new RewaderFactoryMock()))
        ));
